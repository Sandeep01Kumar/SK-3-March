...
- **autocannon version** — captured via `npx --no-install autocannon --version` (e.g., `7.15.0` or a later 7.x release, depending on what `npm install` resolved from `[../../benchmarks/package.json](../../benchmarks/package.json)`'s `^7.15.0` constraint). Recorded because autocannon's default header set and its connection-management behaviour have evolved across major versions; a future major-version bump should be paired with a re-run of the baseline.
