...
- `[../../benchmarks/profile-heap.sh](../../benchmarks/profile-heap.sh)` launches the server with `node --inspect=127.0.0.1:9229 ../server.js` and drives the V8 sampling heap profiler programmatically via the **Chrome DevTools Protocol (CDP)** using `[../../benchmarks/inspector-driver.mjs](../../benchmarks/inspector-driver.mjs)`. The driver issues `HeapProfiler.startSampling` (sampling interval 32 768 bytes), waits for the configured load-duration window to elapse, issues `HeapProfiler.stopSampling`, writes the returned profile as `Heap.<YYYYMMDD>.<HHMMSS>.<server_pid>.0.001.heapprofile`, and then triggers a graceful exit on the inspected process via `Runtime.evaluate({ expression: "process.exit(0)" })`. The sampling cadence is V8's internal allocation-site sampler at the 32 768-byte interval; the resulting profile is interpreted in chapter 04.
...
A CPU-profile harness analogous to the heap-profile harness — `[../../benchmarks/profile-cpu.sh](../../benchmarks/profile-cpu.sh)` — completes the trio. It also launches the server with `node --inspect=127.0.0.1:9229 ../server.js` and drives the V8 sampling CPU profiler via the same `[../../benchmarks/inspector-driver.mjs](../../benchmarks/inspector-driver.mjs)` (this time in CPU mode: `Profiler.enable` → `Profiler.setSamplingInterval(100)` µs → `Profiler.start` → wait → `Profiler.stop`). The returned profile is written as `CPU.<YYYYMMDD>.<HHMMSS>.<server_pid>.0.001.cpuprofile`, and the inspected process is then exited gracefully via `Runtime.evaluate({ expression: "process.exit(0)" })`. The CPU-profile artifact is interpreted in chapter 04.
...
5. **Graceful shutdown**. The shutdown mechanism differs by harness type:
   - For the **baseline harness** (`[../../benchmarks/run-baseline.sh](../../benchmarks/run-baseline.sh)`) and the **event-loop-lag harness** (`[../../benchmarks/measure-event-loop-lag.sh](../../benchmarks/measure-event-loop-lag.sh)`), after the last rung the orchestrator sends `kill -INT $SERVER_PID` and then `wait "$SERVER_PID"` to block until the server exits. **SIGINT (not SIGKILL)** is used as a courtesy convention so that any operator who later inherits the pattern gets a graceful close; for an unmodified `[server.js:L1-L14]` (no custom signal handlers under Constraint C-001), Node.js's default signal action terminates the process and the harness's `wait` collects the signal-based exit status (130 for SIGINT).
   - For the **CPU profile harness** (`[../../benchmarks/profile-cpu.sh](../../benchmarks/profile-cpu.sh)`) and the **heap profile harness** (`[../../benchmarks/profile-heap.sh](../../benchmarks/profile-heap.sh)`), graceful shutdown is performed by `[../../benchmarks/inspector-driver.mjs](../../benchmarks/inspector-driver.mjs)` over the V8 Inspector Protocol: after the driver has written the `.cpuprofile` / `.heapprofile` artifact to disk, it issues `Runtime.evaluate({ expression: "process.exit(0)" })` on the inspected process, then waits 250 ms for the exit to drain. The harness's `wait "$SERVER_PID"` then collects a clean `rc=0`. This mechanism is required because, as the next section explains, V8's `--cpu-prof`/`--heap-prof` flags do **not** flush their artifacts when the process is terminated by default-handled SIGINT — and `[server.js:L1-L14]` cannot be modified to install a custom signal handler under Constraint C-001.
...
The original methodology, drafted in AAP §0.6.2, specified the CPU and heap profile harnesses would invoke `node --cpu-prof ../server.js` and `node --heap-prof ../server.js` and use a SIGINT to flush the profile artifact to disk on shutdown. End-to-end QA verification of the harness at FINAL CHECKPOINT A established that this approach **does not produce the profile artifacts under load against an unmodified `[server.js:L1-L14]`**: the profile files are never written.

The root cause is a documented V8 behaviour:

- The `--cpu-prof` and `--heap-prof` flags arrange for V8 to flush their profile output to disk via the process's `before-exit` / `exit` hooks.
- When a Node.js process has no custom signal handler installed and receives SIGINT (or SIGTERM), the **default signal action** terminates the process abruptly. The V8 exit hooks do **not** run on default-handled signal termination.
- `[server.js:L1-L14]` installs no signal handlers — and cannot be modified to install any under Constraint C-001 (`[README.md:L2]`, `[Section 2.6.2]`). Therefore the harness's SIGINT to the server bypasses V8's profile-flush hook and the artifact is silently lost. The QA report's Issues #1 and #2 documented three independent reproductions of this defect plus a positive control (`process.exit(0)` from inside the same V8 process emits the profile correctly).

The fix preserves C-001 (no source modification) and C-002 (no application-package dependencies) while still emitting the required artifacts. The harness now uses the V8 Inspector Protocol (CDP), which exposes the same V8 sampling profilers via a documented over-the-wire RPC interface:

- **Server invocation**: the harness launches the server with `node --inspect=127.0.0.1:9229 ../server.js &`. The `--inspect` flag is a Node.js runtime flag, not a source change, so Constraint C-001 is preserved.
- **CDP driver**: `[../../benchmarks/inspector-driver.mjs](../../benchmarks/inspector-driver.mjs)` is a small Node-stdlib-only program (`node:net`, `node:crypto`, `node:http`, `node:fs` — no npm dependencies) that connects to the inspector socket, performs the RFC 6455 WebSocket handshake, and dispatches CDP commands. Because it uses only Node stdlib it lives entirely within the harness package and adds zero dependencies to the harness's own `package.json` beyond the existing `autocannon`, preserving the harness package's minimal-footprint posture as well.
- **CPU capture sequence**: `Profiler.enable` → `Profiler.setSamplingInterval(100)` µs (matching V8's `--cpu-prof` default) → `Profiler.start` → wait for the configured load-duration window → `Profiler.stop` (returns the profile object) → write to `CPU.<YYYYMMDD>.<HHMMSS>.<server_pid>.0.001.cpuprofile` → `Runtime.evaluate({ expression: "process.exit(0)" })` → drain 250 ms → close WebSocket.
- **Heap capture sequence**: `HeapProfiler.startSampling` with `samplingInterval=32768` bytes (matching V8's `--heap-prof` default) → wait for the configured load-duration window → `HeapProfiler.stopSampling` (returns the profile object) → write to `Heap.<YYYYMMDD>.<HHMMSS>.<server_pid>.0.001.heapprofile` → `Runtime.evaluate({ expression: "process.exit(0)" })` → drain 250 ms → close WebSocket.
- **Filename convention**: the new filenames embed the date, time, server PID, isolate id `0`, and sequence `001` so that an operator running multiple captures in the same `results/<timestamp>/` directory gets stable, sortable, and unambiguous names. The `.cpuprofile` and `.heapprofile` extensions are preserved verbatim so Chrome DevTools' "Load Profile…" can consume the files without renaming.
- **Defence-in-depth (Option A)**: after the driver returns, the profile harness asserts that the expected `.cpuprofile` / `.heapprofile` file exists and is non-empty; if not, the script exits with code 5 (profile capture failed) and the operator sees the failure immediately — no longer is a silent miss buried inside a `profile_files: []` field of a JSON manifest.

The end-to-end effect is that the profile harnesses now produce the artifacts the QA report (FINAL CHECKPOINT A) and chapter 04 ("CPU and Memory Profile") both require, while every constraint in AAP §0.7 ("Rules") remains satisfied. This is the QA-recommended "Option C" path: stay within the C-001 boundary and use a programmatic alternative to default-signal termination.
