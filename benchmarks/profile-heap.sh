#!/usr/bin/env bash
# benchmarks/profile-heap.sh
#
# Purpose
# -------
# Capture V8 sampling heap profile of ../server.js under sustained load. The
# script launches `node --inspect=127.0.0.1:9229 ../server.js` (which exposes
# the V8 Inspector socket on the loopback interface), then runs the
...
# programmatically starts the V8 sampling heap profiler via
# HeapProfiler.startSampling, sleeps for a fixed window that brackets the
# autocannon load period, then reads back the profile data via
# HeapProfiler.stopSampling and writes it to a .heapprofile file in
# $RESULTS_DIR. Finally the driver asks the inspected process to exit
# cleanly via Runtime.evaluate (`process.exit(0)`), which lets the harness
# reap the server PID without any signal-handling races.
#
# AAP Reference
# -------------
# AAP §0.6.2 — `benchmarks/profile-heap.sh`.
#
# Why CDP-driven Profile Capture (root cause of QA Issue #2)
# ----------------------------------------------------------
# An earlier revision of this script launched the server with the V8
# `--heap-prof` runtime flag and used SIGINT for shutdown. That mechanism
...
# `before-exit` / `exit` hooks run and flush the .heapprofile file. When
...
# and the V8 exit hooks do NOT run, so no .heapprofile is written. QA
# verified this by direct experiment (Test E in the QA report).
#
...
# via HeapProfiler.stopSampling, which returns the same JSON shape that
# --heap-prof would have written, but it requires no graceful-exit timing.
# Once the profile is safely on disk, the driver issues Runtime.evaluate(
# `process.exit(0)`) so the server exits cleanly and the harness can
# reap it with a normal `wait`. This approach is QA-recommended Option C
# ("READY — no constraint relaxation needed") per the QA report.
#
# Symmetry with profile-cpu.sh
# ----------------------------
# This script is the structural twin of profile-cpu.sh. The only differences
# are the inspector-driver --mode flag (heap vs cpu), the profile filename
# glob (Heap.*.heapprofile vs CPU.*.cpuprofile), the autocannon output
# filename, the manifest filename, and the echo strings. Any other
# divergence between the two files is a defect.
#
# Constraints Honored
# -------------------
...
#   5 — profile capture failed (driver did not produce a non-empty .heapprofile)
...
# --------------------------------------------------------------------
#   Heap.<YYYYMMDD>.<HHMMSS>.<server_pid>.0.001.heapprofile
#                                                V8 sampling heap profile
#                                                artifact (same JSON shape
#                                                as the V8 --heap-prof flag
#                                                would emit; consumable by
#                                                Chrome DevTools Memory tab).
#   autocannon-during-heapprof.json              load generator's machine-readable report
#   heap-profile-manifest.json                   run metadata (timestamp, node version, params)
...
# populate the heap profile's allocation-site samples with representative data
...
# Compute the .heapprofile output filename in the same `Heap.<date>.<time>.<pid>.<id>.<seq>.heapprofile`
# pattern that V8's `--heap-prof` flag would have used. Using the same naming
# convention means Chrome DevTools and any downstream tooling that globs for
# `Heap.*.heapprofile` files keeps working without changes.
...
# ---------------------------------------------------------------------------
# Launch server with V8 Inspector enabled (no --heap-prof flag — the profile
# is captured by the inspector-driver helper via the CDP HeapProfiler domain).
...
PROFILE_FILE="$RESULTS_DIR/Heap.${DATE_TAG}.${SERVER_PID}.0.001.heapprofile"
...
# $HOST:$INSPECTOR_PORT, starts the V8 sampling heap profiler, sleeps for
# $PROFILE_DURATION_MS (which is LOAD_DURATION + PROFILE_TAIL_S seconds),
# stops the profiler, writes the .heapprofile JSON to $PROFILE_FILE, then
...
# autocannon begins firing requests.
...
( cd "$BENCH_DIR" && npx --no-install autocannon \
    -c "$LOAD_CONNECTIONS" \
    -d "$LOAD_DURATION" \
    -j "$URL" ) > "$RESULTS_DIR/autocannon-during-heapprof.json"
echo "Load complete."
...
# produce a non-empty .heapprofile, surface the failure to the operator and
...
if [ ! -s "$PROFILE_FILE" ]; then
  echo "ERROR: No heap profile was produced at $PROFILE_FILE" >&2
...
echo "Emitted heap profile: $PROFILE_FILE (${PROFILE_SIZE_BYTES} bytes)"
...
# ---------------------------------------------------------------------------
# Manifest — record run metadata and the list of emitted .heapprofile files.
# Uses fs.readdirSync to enumerate emitted profiles and fs.writeFileSync to
# atomically persist the manifest JSON.
# ---------------------------------------------------------------------------
NODE_VERSION="$(node --version)"
MANIFEST_FILE="$RESULTS_DIR/heap-profile-manifest.json"
node -e '
  ...
  let profileFiles = [];
  try {
    profileFiles = readdirSync(heapProfDir)
      .filter((name) => name.startsWith("Heap.") && name.endsWith(".heapprofile"))
      .sort();
  } catch (_e) {
    profileFiles = [];
  }
  const manifest = {
    ...
    heap_profile_dir: heapProfDir,
    profile_files: profileFiles,
    capture_mechanism: "v8-inspector-protocol",
    notes: "Heap profile captured via V8 Chrome DevTools Protocol (HeapProfiler.startSampling \u2192 HeapProfiler.stopSampling) driven by benchmarks/inspector-driver.mjs. The .heapprofile JSON is the same shape that the --heap-prof runtime flag would have emitted. Inspect with Chrome DevTools (Memory tab \u2192 Load profile). An empty profile_files array indicates the driver failed; check inspector_driver_log for diagnostics.",
  };
...
' \
  ...
  "$LOAD_DURATION" \
  "$PROFILE_DURATION_MS" \
  "$RESULTS_DIR/autocannon-during-heapprof.json" \
...
echo "Heap profile capture complete. Open with Chrome DevTools (Memory tab → Load profile)."
