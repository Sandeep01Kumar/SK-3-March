...
# Purpose
# -------
# Capture V8 heap profile of ../server.js under sustained load. The script
# launches `node --heap-prof --heap-prof-dir=$RESULTS_DIR ../server.js` (which
# writes a Heap.<pid>.<id>.<seq>.<thread>.heapprofile to $RESULTS_DIR when the
...
# .heapprofile is collected into the timestamped results directory.
#
# AAP Reference
# -------------
...
# ----------------------------
# This script is the structural twin of profile-cpu.sh. The only differences are
# the diagnostic flag (--heap-prof vs --cpu-prof), the directory variable name
# (HEAP_PROF_DIR vs CPU_PROF_DIR), the profile filename glob (Heap.*.heapprofile
# vs CPU.*.cpuprofile), the autocannon output filename, the manifest filename,
# and the echo strings. Any other divergence between the two files is a defect.
#
# Constraints Honored
# -------------------
...
# --------------------------------------------------------------------
#   Heap.<pid>.<id>.<seq>.<thread>.heapprofile   V8 heap profile artifact(s)
...
# --heap-prof writes the .heapprofile into HEAP_PROF_DIR. Point it directly at
# RESULTS_DIR so no post-hoc move is required.
HEAP_PROF_DIR="$RESULTS_DIR"
...
# --heap-prof flag only writes the profile on graceful exit; SIGKILL would
...
# ---------------------------------------------------------------------------
# Launch server with V8 heap profiling enabled
# ---------------------------------------------------------------------------
echo "Launching server: node --heap-prof --heap-prof-dir=$HEAP_PROF_DIR $SERVER_SCRIPT"
node --heap-prof --heap-prof-dir="$HEAP_PROF_DIR" "$SERVER_SCRIPT" \
...
# ---------------------------------------------------------------------------
# Stop the server with SIGINT to trigger heap profile emission. SIGKILL would
# discard the profile because V8 only writes the .heapprofile via its exit
...
# ---------------------------------------------------------------------------
echo "Stopping server to emit heap profile..."
if kill -0 "$SERVER_PID" 2>/dev/null; then
  kill -INT "$SERVER_PID" 2>/dev/null || true
  wait "$SERVER_PID" 2>/dev/null || true
fi
...
echo "Emitted heap profiles:"
ls -1 "$HEAP_PROF_DIR"/Heap.*.heapprofile 2>/dev/null || echo "  (none — see $RESULTS_DIR/server.stderr.log)"
...
node -e '
  ...
  const manifest = {
    ...
    notes: "Heap profile emitted by node --heap-prof on graceful (SIGINT) shutdown. Inspect with Chrome DevTools (Memory tab \u2192 Load profile) or speedscope. An empty profile_files array indicates the server did not run V8 exit hooks before terminating (see server.stderr.log).",
  };
