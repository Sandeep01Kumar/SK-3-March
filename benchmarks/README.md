...
- **C-002 (Zero application dependencies)** — All dev-dependencies (`autocannon`, optional `clinic`, optional `0x`) are declared only in `benchmarks/package.json` and resolve only into `benchmarks/node_modules/`. The application's root `package.json` and root `package-lock.json` are never modified. This boundary is why `npm install` must be run from inside `benchmarks/` and never at the repository root.
